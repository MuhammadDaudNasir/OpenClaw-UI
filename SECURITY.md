...
If you discover a security vulnerability in OpenClaw UI, please report it responsibly:
...
OpenClaw UI runs entirely on your local machine. Key security properties:

- **No cloud backend** — all OpenClaw interaction goes through the local `claude` CLI.
...
No other network connections are made by OpenClaw UI itself. The `openclaw` CLI may make its own connections as part of normal operation.
